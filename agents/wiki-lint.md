...
3. **Run all checks** in a single pass over the inventory + link map:
   ...
   - Temporal validity — for each page declaring `valid_from` / `valid_through`, whether the window is readable and whether it covers today (`src/helpers/temporal-validity-lint.mjs`, `lintTemporalValidity`, reference day resolved ONCE per pass). A page with no window produces nothing: the check never asks for one. Every page type is checked, not just decisions.

   - Conventions drift — for each convention installed in the vault's `CLAUDE.md`, whether its section still matches the snippet of the same identity (`src/helpers/convention-drift.mjs`, `auditConventionDrift` with `governed: false`). Report `drift` with its line count and `duplicate-identity`; `not-installed` is a choice, not a defect. Read every `CLAUDE.md` the vault has — a vault with two carries two rule sets and only one is being read.

4. **Surface findings** by severity:
   - ERROR: dead wikilinks, stale index entries, `session-folder-collision`, `valid-window-inverted`
   - WARNING: orphans, missing index entries, frontmatter gaps, empty sections, `session-folder-stray`, `catalog-sessions-heading`, `prompt-status-missing`, `prompt-status-invalid`, `convention-drift`, `valid-window-unreadable`
   - INFO: log out-of-order, hot stale, `valid-window-not-yet`, `valid-window-expired` (a page describing a period now past is still knowledge — say the state, never ask for a fix)
