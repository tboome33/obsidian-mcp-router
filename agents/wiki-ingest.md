...
tools: Read, Glob, Grep, WebFetch, mcp__obsidian-router__list_vaults, mcp__obsidian-router__list_files, mcp__obsidian-router__get_file, mcp__obsidian-router__write_file, mcp__obsidian-router__patch_file, mcp__obsidian-router__append_to_file, mcp__obsidian-router__merge_frontmatter, mcp__obsidian-router__search, mcp__obsidian-router__search_smart
---
...
3. **Check** which entity/concept pages already exist (read `wiki/index.md` from the target vault).
...
6. **Patch** `wiki/index.md` to add rows for newly created pages.
7. **Append** a single entry to `wiki/log.md` describing this ingestion.
8. **Do NOT** touch `wiki/hot.md` — the orchestrator does that once after all agents return (avoids race).
