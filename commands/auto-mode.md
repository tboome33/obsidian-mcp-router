---
description: |
  Set the wiki auto-enrichment mode for the current session. Auto-enrichment is the layer where Claude proactively proposes wiki saves at three triggers (validation pins, result-obtained digests, topic-switch checkpoints). The mode controls how much friction the user sees. Pass "persist" to write OBSIDIAN_ROUTER_AUTO_ENRICH=<mode> to the workspace `.env` so the choice survives a Claude Code restart.

  Four modes, each fits a different posture:
  ...
  - `Hybrid` (auto-save type-safe like facts/URLs, ask on decisions/ADRs/techniques/rules). Best for: power-user sweet spot after a calibration week, active development with frequent URL ingestion, research with citations to pile up but conclusions to vet.
  ...
  EN triggers: "switch to <mode> mode", "set auto-enrich to <mode>", "use ClaudeAsk / Hybrid / FullAuto / off", "I want Claude to ask before saving" (→ ClaudeAsk), "auto-save the cheap stuff" (→ Hybrid), "save everything automatically" (→ FullAuto), "stop auto-saving" / "disable auto-enrichment" (→ off).
  FR triggers : "passe en mode <mode>", "mets l'auto-enrichissement sur <mode>", "utilise ClaudeAsk / Hybrid / FullAuto / off", "je veux que Claude demande avant de sauver" (→ ClaudeAsk), "auto-save les trucs sûrs" (→ Hybrid), "sauve tout automatiquement" (→ FullAuto), "arrête de sauver auto" / "désactive l'auto-enrichissement" (→ off).
