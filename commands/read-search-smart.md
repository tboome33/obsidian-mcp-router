...
Pre-requisites: the target vault must have BOTH the `mcp-tools` and `smart-connections` plugins enabled, with Smart Connections having indexed the vault. If the tool returns 503 saying smart-connections isn't available, surface that clearly and tell the user how to install it.
