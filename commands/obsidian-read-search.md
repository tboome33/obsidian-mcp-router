...
Argument parsing:
- the entire $ARGUMENTS line as `query` is fine for the most common case
...
If many matches (>20), summarize counts and show the first 20. Suggest `search_smart` (semantic) if the user seems to be looking by meaning rather than literal text.
