...
6. **Conventions picker** via `AskUserQuestion multiSelect`: the 8 installable conventions (`roadmap-discipline`, `default-vault-health-check`, `wiki-query-first`, `path-disambiguation` pre-checked; `source-type`, `languages`, `heading-hierarchy`, `auto-enrichment` recommended). `languages` then asks the vault's languages (ISO 639-1 codes, `fr` by default) and is installed with that value; the retired `bilingual` is offered as a migration, never as a box. Installs via `/obsidian-router:conventions install <id>` so the H2-heading idempotency guard fires.
