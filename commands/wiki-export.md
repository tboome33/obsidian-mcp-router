...
Default behaviour:
- Target = `llms` (compact `llms.txt`). Override with `--target full` for `llms-full.txt` (page bodies inlined).
- Output = `wiki-meta/exports/<vault>-<target>-<ISO-date>.txt` inside the vault. Override with `--stdout` to return the content in the chat instead.
- Read-only: never mutates wiki pages. Only writes the export file + appends a log entry.
