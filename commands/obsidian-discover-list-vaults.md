---
description: List all configured Obsidian vaults with their online status, latency, and any issues.
---
...
- `defaultVault` — the vault used when calls omit a `vault` argument
...
Render the result as a compact markdown summary:
- One status line: `<n> vault(s) configured · <m> online · default: <name>`
- A table with columns: name | type | status (✅/⚠️/❌) | latency | path or baseUrl
- Highlight any vault that is offline, has `missingApiKey: true`, or returns an error
- For unhealthy vaults, suggest the matching obsidian-router-status skill or a concrete fix
