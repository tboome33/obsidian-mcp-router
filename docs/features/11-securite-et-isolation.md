...
| `OBSIDIAN_ROUTER_READONLY=true` | **Lecture seule** : les 17 outils d'écriture (dont `download_page_assets` et `pptx_extract_assets`, qui écrivent des fichiers sur disque) sont retirés de la liste des outils **et** refusés à l'appel — même un client qui connaît le nom de l'outil et l'appelle directement est bloqué. |
