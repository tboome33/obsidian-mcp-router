...
The router's catalog holds **42 MCP tools**: vault discovery & session control (5), reading & search (4), writing & surgical editing (7), Templater execution (1), document conversion to markdown (12), web-ingestion helpers (3), wiki graph & context (5), links & navigation (3), and the vault-creation wizard (2). The full, human-readable feature guide lives in [`docs/features/`](features/README.md); the compact per-tool tables are in the [README](../README.md).
