...
The router's catalog holds **55 MCP tools**: vault discovery & session control (7), reading & search (4), the local BM25 search index (1), writing & surgical editing (8), Templater execution (1), document conversion and asset extraction (13), web-ingestion helpers (4), the source ledger (2), the conventions audit (1), wiki graph & context (7), OKF projections (1), links & navigation (3), and the vault-creation wizard (3). The full, human-readable feature guide lives in [`docs/features/`](features/README.md); the compact per-tool tables are in the [README](../README.md).
