...
**What holds today — and it is no longer advice.** On a vault this router's own binding registry sees attached to **more than one workspace**, a write with no precondition is **refused**, not merely discouraged. The requirement is computed, never declared: there is no switch to set and nothing to keep up to date, because the registry already knows which workspaces name which vault. A vault only one workspace declares is completely unaffected — it writes exactly as it did before, `ifMatch` optional.

Three details worth knowing before you meet the refusal:

- **What satisfies it.** `ifMatch` (the `contentSha256` a `get_file` returned) on `write_file`, `append_to_file`, `patch_file`, `set_frontmatter`, `merge_frontmatter`, `move_file` and `delete_file`; `ifNew: true` on `write_file` when you are creating a note that must not exist yet — that is a compare-and-swap against absence, and it is why creating pages on a shared vault stays possible. A `write_bundle` needs one **per step**; it checks them all before its first write, so a stale bundle refuses whole.
- **`openVaults` counts as shared by hypothesis.** Such a vault is reachable from every workspace without being declared by any, so the count cannot see it and its readership is not knowable. It always requires a precondition.
- **Where to look before you write.** `list_vaults` reports `writesRequireIfMatch` and `sharingReason` per vault, so a session can know in advance rather than by being refused.

Tools that regenerate a derived artifact from the vault's own content (`build_wiki_graph`, `build_search_index`, `refresh_okf_projections`) are exempt: there is no "content I read" to pin, and two sessions racing produce the same file from the same source. `record_source` is exempt because it already does its own compare-and-swap on the shared ledger. `execute_template` with `createFile: true` carries no precondition at all, so on a shared vault it is refused with the way through: render it without `createFile` and write the result with `write_file`.

**The honest limit, stated with the mechanism rather than left implicit.** `ifMatch`'s compare-and-swap only protects writes that go through this same route. It says nothing about a concurrent edit made straight in Obsidian's own editor on the machine that hosts the vault, or a parallel Obsidian Sync / LiveSync pass — those never touch the REST API this router calls, so no `ifMatch` check ever sees them. Two narrower ones, for the same reason: `move_file`'s precondition guards the **source**, so with `overwrite: true` the destination can still be replaced (the default, `overwrite: false`, refuses an existing destination outright); and the router's own maintenance writes — the audit line, the first-contact repair, the projections refresh — carry no precondition, being regenerations rather than edits.
