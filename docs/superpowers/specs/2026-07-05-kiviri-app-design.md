...
| D6 | Modèle de coût | 3 seaux : (A) inférence LLM, (B) compute spécialisé, (C) hosting. **Managed + ingestion lourde = TOUJOURS payants** ; c'est **nous qui choisissons les modèles** par fonctionnalité. Self-host quand le coût est fixe (embeddings de la recherche sémantique, OCR sur GPU Dedibox), API tierces sinon. |
| D7 | BYO-key | **Réservé aux abonnés payants — ce n'est PAS une voie pour éviter l'abonnement** (précision Roland 2026-07-05) : pour l'abonné qui dépasse son forfait et **ne veut pas monter de niveau** (X1→X5→X20), **ou qui est déjà en X20**. À ne pas confondre avec le **mode MCP** (le Claude de l'user, côté client — c'est ça, le gratuit). **Pas de BYO-key sur le gratuit** ; **jamais de BYO-key sur l'ingestion lourde** (modèles spécialisés multiples = trop complexe). |
| D8 | Offre gratuite | **UNE seule — un compte Personal solo au niveau gratuit** (« Kiviri Free », précisé 2026-07-05) : 1 utilisateur, 1 vault, petit quota (200 Mo par défaut). **L'application y est un viewer + modifications MANUELLES uniquement** : l'éditeur à blocs complet pour écrire à la main, la recherche sémantique locale incluse (self-hostée, coût fixe), l'import dans le quota — mais **AUCUNE fonction IA dans l'application** (pas d'assistant, pas d'ingestion, pas de RAG, pas de BYO-key). L'IA du gratuit = **le propre Claude de l'utilisateur via MCP** (son compte Claude personnel, côté client). Ouverture possible d'une IA gratuite le jour où une infra open-source à coût connu existe (ex. GLM). |
...
| D15 | Collaboration vivante | Panneau accordéon **« qui est en ligne + sur quoi il travaille »** (2 niveaux de résumé : fichiers touchés [auto, gratuit] / résumé IA [option]) · **chat intégré** entre membres du vault · **détection IA de convergence de sujets** (via les embeddings de la recherche sémantique locale) qui invite deux utilisateurs à discuter. → Release 2. |
...
| D21 | Limites & cascade hors-forfait | **Limites hebdomadaires** par famille de capacité (assistant courant vs capacités lourdes), avec barres de progression + date de réinitialisation visibles (UX à la Claude, capture de référence 2026-07-05). Cascade au dépassement : l'UX propose **d'abord la montée de niveau** (X1→X5→X20), puis **crédits d'utilisation** (opt-in, à la consommation), puis **BYO-key** (abonnés seulement, D7). Jamais de blocage brutal. |
...
| D25 | Mode fusion (débat multi-LLM) | Fonctionnalité avancée payante (opération premium, comptée cher dans le quota) : sur un sujet complexe, **3 LLM débattent** (réponses indépendantes → critiques croisées anonymisées, N tours) et un **juge tranche**. **L'orchestrateur = NOTRE code** dans le backend Kiviri (boucle modeste, pattern public type `llm-council` de Karpathy) ; **juge = GLM 5.2 via API** (précisé 2026-07-05 : le self-host est irréaliste — modèle MoE géant, infra hors de prix ; l'API GLM est parmi les moins chères du marché, donc le rôle qui lit le plus de tokens reste sur le meilleur rapport qualité/prix ; ⚠️ **API commerciale, PAS un abonnement grand public** — même logique zéro-ToS que pour Obsidian ; l'abonnement GLM reste pour l'usage perso de Roland) ; **passerelle modèles = OpenRouter en simple tuyau** (une intégration pour tous les modèles, n'orchestre rien, interchangeable). **Différenciateur : le débat est ancré dans le vault** (les débatteurs reçoivent le contexte des notes via la recherche sémantique locale) et le **verdict est filable en note** avec les arguments des deux camps. Panels/modèles/nb de tours configurés dans Kiviri Control. Le serveur Fusion perso de Roland reste une inspiration UX (presets, fourchettes de coût, délibération asynchrone), pas une dépendance. |
...
**Track A — App-cœur** : store + adaptateur router → import (Obsidian + OKF) → éditeur à blocs + arborescence + wikilinks/backlinks + tags → recherche (texte + sémantique locale) → collaboration niveau 1 → Kiviri Sync → graphe → édition simultanée (Enterprise) → polish/mobile/sécurité.

**Track B — Capacités IA** (chaque pipeline = un chantier ; priorisées du plus proche de l'existant au plus lourd) :
- **IA-1** : mémoire always-on + ask-your-vault (RAG : embeddings locaux + LLM) + ingestion de base (web/YouTube/PDF, outils existants) + auto-liaison.
...
- **IA-6** : **mode fusion** (D25) — débat multi-LLM arbitré, ancré dans le vault, verdict filable en note. Orchestrateur maison + juge GLM via API + OpenRouter en tuyau.
...
**Releases 3+** : IA-3 (audio/vidéo) · IA-4 (voix) · **IA-6 (mode fusion)** · graphe · édition simultanée Google Docs (Enterprise) · pages Kiviri à mise en page libre · export OKF payant · backups externes HDS.
