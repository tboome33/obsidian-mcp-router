...
> **One exception, since router v0.89.0: `FullAuto` never comes from a workspace `.env`.** A workspace is very often a cloned repository whose `.env` arrived with it, and `FullAuto` is the one mode that turns such a file into standing permission to write into this vault without asking again. So a value that canonicalises to `FullAuto` (`fullauto`, `full`, `full-auto`, `auto`, any casing) read from a workspace `.env` is **not applied**, and `--persist` refuses to write it — the mode still applies to the session, and the refusal says where to set it instead. `ClaudeAsk`, `Hybrid` and `off` are unaffected in every respect. `FullAuto` still comes from the MCP host's server declaration or from `/obsidian-router:auto-mode FullAuto` in the session. If `list_vaults` returns a non-null `autoEnrichModeRefused`, tell the user their project's file asked for that mode and did not get it.
