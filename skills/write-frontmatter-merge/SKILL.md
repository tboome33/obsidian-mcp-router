...
If the call fails entirely (not a per-key partial failure), do NOT silently redo it with direct-filesystem tools (`Read`/`Edit`/`Write` on the vault's real path):
