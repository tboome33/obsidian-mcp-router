...
1. **The current Claude session is bound to this vault** — `list_vaults` reports a `workspaceBinding` naming it (the confirmed binding in the user's own config, which is what decides), OR the session's working directory IS this vault and its own `.env` carries `VAULT_PATH` pointing at that same directory (the "this folder is a vault" case `setup-vault` writes), OR the user explicitly opted in this session ("track this in <vault>", "prends des notes dans mon vault"), OR the conversation runs inside a Claude Desktop Project bound to this vault. Two things that are NOT enough on their own, because the router no longer applies either from a project file: a bare `OBSIDIAN_ROUTER_DEFAULT_VAULT` line, and a `VAULT_PATH` in some OTHER project's `.env` that points here. Both are proposals the router reports and does not apply — `list_vaults` shows the first as `bindingHint`. Read `workspaceBinding`; do not infer the binding from a variable.
