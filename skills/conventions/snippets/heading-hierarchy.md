...
| `valid_from` / `valid_through` | when the ruling itself is dated | Not part of this contract, and not decision-specific: any page may declare when what it says applies. A decision can be `accepted` **and** no longer in force. Rules in the `temporal-validity` convention — do not restate them here. |
