...
### 2b-ter. Check O: two folders named "Sessions" (v0.92.0+)

Purely structural, one `list_files` away, and it catches a drift no other check sees: **the same content type living in two directories at once**.

`wiki-meta/Sessions/` is where session notes go — one file per Claude Code session, written by the `session-auto-journal` hook, and their home since v0.12.8 moved them out of `wiki/`. Anything under `wiki/Sessions/` (any case) is a second, competing home.

**Preferred path (v0.92.0+):** the `refresh_okf_projections({ vault, check: true })` call Check L already makes returns a `sessions` block — `findings[]` (each with `rule`, `severity`, and then either `wikiFiles`/`metaFiles`/`detail` for a folder rule, or `file`/`line`/`heading` for the catalogue rule) or `skipped`. Read it from there; the marker rule below is applied by the router from the same bytes the projection planner used. Fall back to the manual walk only when that tool is unavailable:

**Reading `skipped` — two values, two very different meanings.** `enumeration-failed` / `enumeration-truncated`: the vault could not be listed in full, `findings` is empty, and the honest report is "Check O could not run" rather than "no collision". `catalog-read-failed`: only the catalogue could not be read, so the *heading* question below is open — but any findings listed alongside it were genuinely established and must still be reported. Never drop a finding because `skipped` is set.

1. `list_files({ vault, directory: 'wiki' })` and `list_files({ vault, directory: 'wiki-meta' })`; find directories whose name is `Sessions` case-insensitively (`sessions` is the pre-v0.12.8 spelling and still on disk in several vaults).
2. Enumerate the `.md` files under each, recursively. For a file named `index.md` or `log.md`, **read it and ignore it only if its body carries the `> Generated by obsidian-mcp-router` marker** — a generated projection outlives the content that caused it, so counting one flags exactly the vaults whose misfiled note was just moved away. The reserved *basename* is not enough: this repo treats an UNMARKED file at a reserved path as user-owned (that is why `refresh_okf_projections` reports it as a `conflict` instead of overwriting it), and a hand-written `wiki/Sessions/index.md` is content that must be reported.
3. Content under `wiki/Sessions/` **and** under `wiki-meta/Sessions/` → **ERROR** `session-folder-collision`. Content under `wiki/Sessions/` only → **WARNING** `session-folder-stray` (a pre-v0.12.8 vault whose migration never ran, or a misfile in a vault the hook has not written to yet).
4. **The heading, not only the folder** — **WARNING** `catalog-sessions-heading`: the vault's `wiki-meta/catalog.md` (or the pre-0.58.0 `index.md`) still carries a `## Sessions` AREA heading. This is the *cause* rather than the symptom — the catalogue answering "where does a session note go?" with a folder that must not hold them — and it fires in vaults where nothing has been misfiled yet, which is why the folder rules alone called 16 fleet catalogues clean on 2026-09-07. The tool reports the `file`, the 1-based `line` and the raw `heading` text; a heading is only counted where markdown makes it one, so an example inside a fenced code block is not a finding.

**Report; never propose a move as the fix.** The `wiki/` files are frequently *curated* pages — an incident write-up, a project recap — not raw logs. Folding those into `wiki-meta/Sessions/` would bury them: that folder is in `DEFAULT_EXCLUDE_FOLDERS` and drops out of the search corpus. The right destination is a real content area (`wiki/Incidents/`, `wiki/Projects/`, `wiki/Daily/`), and only the user can say which. When rule 4 above also fired on the same vault, pair the two in your report: the heading is what sent the file there, and removing it is the durable half of the repair — the one mechanical fix this check does offer.

The same rules exist deterministically in `src/helpers/session-folder-collision.mjs`, which the repo's offline fleet CLI (`okf-projections`, `--all-vaults`) uses to sweep every vault at once with Obsidian closed. That is the operator's tool, not this skill's — wiki-lint stays read-only over the router tools. Apply the rules above as written; never write a second implementation of them.
...
### 2d-ter-bis. Check S: which conventions is this vault under (2026-09-26+)

Check Q compares a section that is THERE with its snippet. It cannot see a vault with two conventions files (the router then reads none), a convention that was never installed, or a backup that is somebody else's history. Call `audit_vault_conventions({ vault })` — read-only, over REST, remote vaults included — and fold its findings into the report:

- `ambiguous-conventions-file` → **ERROR** (the vault's rules are in force for nobody). Quote the audit's repair: when one file is a verbatim template copy, the `move_file` that renames it (with its `ifMatch`) — to PROPOSE, never to run inside the lint.
...
**Exception — Check O's `session-folder-collision` is an ERROR with no offered fix.** Its "proposed fix" column reads: *the user names the content area these pages belong in; nothing is moved until then.* The only mechanical repair you may offer is removing the `## Sessions` area heading a `catalog-sessions-heading` finding names (it carries the file and the exact line) — that heading is what sends the next agent to the wrong folder, and it is a WARNING you may fix even though the collision beside it is an ERROR you may not. An agent reading this skill cold flagged the tension between "propose a fix for every ERROR" and "never propose a move"; this paragraph is the resolution, and it is deliberate: moving a curated page into `wiki-meta/Sessions/` buries it (the folder is excluded from search), so a wrong guess costs more than an open question.
