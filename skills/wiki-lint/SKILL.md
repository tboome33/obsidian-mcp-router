...
**Preferred path (v0.92.0+):** the `refresh_okf_projections({ vault, check: true })` call Check L already makes returns a `sessions` block — `findings[]` (each with `rule`, `severity`, `wikiFiles`, `metaFiles`, `detail`) or `skipped` (`enumeration-failed` / `enumeration-truncated`: the vault could not be listed in full, so report "Check O could not run" rather than "no collision"). Read it from there; the marker rule below is applied by the router from the same bytes the projection planner used. Fall back to the manual walk only when that tool is unavailable:
...
**Exception — Check O's `session-folder-collision` is an ERROR with no offered fix.** Its "proposed fix" column reads: *the user names the content area these pages belong in; nothing is moved until then.* The only mechanical repair you may offer is removing a `## Sessions` area heading from `wiki-meta/catalog.md` when one is present — that heading is what sends the next agent to the wrong folder. An agent reading this skill cold flagged the tension between "propose a fix for every ERROR" and "never propose a move"; this paragraph is the resolution, and it is deliberate: moving a curated page into `wiki-meta/Sessions/` buries it (the folder is excluded from search), so a wrong guess costs more than an open question.
