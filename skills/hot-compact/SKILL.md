...
1. **Mesurer sans polluer le contexte.** Si `get_file` sur `wiki-meta/hot.md` risque de dépasser le plafond de contexte (fichier > ~20 Kio), NE PAS le charger en entier dans la conversation : passer par un script (Node) qui importe les modules du router lui-même (`src/registry.mjs` → `loadRegistry`, `src/rest-client.mjs` → `getFileContent`) et ne remonte que taille, `contentSha256` et structure (titres, dates des entrées, N premiers blocs). **Jamais d'appel direct à l'API Local REST pour écrire, et jamais d'écriture filesystem** : une écriture qui contourne le router échappe à toutes ses gardes (paliers d'écriture, vault partagé, audit) et est exactement l'écrivain non-CAS contre lequel la limite honnête d'`ifMatch` met en garde.
...
6. **Contrôle de concurrence** : passer à l'écriture le `contentSha256` lu à l'étape 1 comme `ifMatch` — le router refuse (409) si l'original a changé depuis (autre session) ; dans ce cas **abandonner et recommencer**, ne jamais écraser une version non lue. Sur un vault que plusieurs workspaces partagent (`list_vaults` → `writesRequireIfMatch: true`), `ifMatch` n'est pas un choix : sans lui l'écriture est refusée.
7. **Écrire** le hot mince via `write_file` du router avec `ifMatch` (laisse la trace MCP que le guard sait voir) ; le backup de l'étape 2 s'écrit avec `ifNew: true`.
