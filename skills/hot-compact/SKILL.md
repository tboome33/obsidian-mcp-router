...
3. **Sélectionner le contenu du hot mince** : les 3-6 faits les PLUS RÉCENTS (l'ordre du fichier se détecte par les dates — ne pas supposer), les chantiers actifs, et **obligatoirement** tout bloc épinglé (marqueur `📌`). Ce qui sort du cache reste retrouvable : backup + `log.md` + `Sessions/` + notes liées.
...
8. **Tracer** : entrée mince dans `wiki-meta/log.md` (`## <date> — hot.md compacté · [[hot.full-backup-<date>]]` + 1 phrase FR + 1 phrase EN, tailles avant/après).
