...
- If `bindingHint` has a signalled status (`unconfirmed`, `unknown-vault`, `conflicts`), the project proposed a vault that nobody answered. Say so in one sentence — *"This project's .env proposes `<hint>`; it was not applied."* (name the host instead when `origin` is `host`) — and let the wizard answer it: binding `<hint>` as the primary adopts it; if the user says they do NOT want it, call `confirm_workspace_binding({ refuse: "<hint>" })` and relay the result (`hintWritten` says whether the project's `.env` received the portable `OBSIDIAN_ROUTER_REFUSED_VAULT` line beside the proposal it answers). `previouslyRefused: true` means the file says it was refused here before and the config has no answer any more — say that, ask once. A `refused` status is silence: do not bring the vault up again.
- Otherwise continue.
