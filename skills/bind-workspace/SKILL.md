...
- If `bindingHint` has a signalled status (`unconfirmed`, `unknown-vault`, `conflicts`), the project proposed a vault that has no answer yet. Say so in one sentence — *"This project's .env proposes `<hint>`; it was not applied."* (name the host instead when `origin` is `host`) — then let the wizard answer it. `unconfirmed`: binding `<hint>` as the primary adopts it. `conflicts`: the workspace is already bound elsewhere, so adopting means re-binding it — Step 2 asks. `unknown-vault`: there is nothing to bind; the vault must be registered first (`setup-vault`), or refused. If the user does NOT want it, call `confirm_workspace_binding({ refuse: "<hint>" })` and relay the result: `silencesCurrentHint`, `hintWritten` (whether the project's `.env` received the portable `OBSIDIAN_ROUTER_REFUSED_VAULT` line beside the proposal it answers), and `hintError` when that half failed (the refusal is then in force from the config alone). `previouslyRefused: true` means the file, as loaded at start-up, says it was refused here before and no refusal is recorded in the config any more — say that, ask once. A `refused` status is silence: do not bring the vault up again. A proposal can come from either of the file's two lines — its `OBSIDIAN_ROUTER_DEFAULT_VAULT`, or its own `OBSIDIAN_ROUTER_LOCKED` when it carries no default — so a project that only locks is proposing too, and the refusal answers it the same way. `bindingHint.byLock: true` says the proposal came from the LOCK line: accepting it means `confirm_workspace_binding({ vault: "<hint>", locked: true })`, and you must say that accepting restricts the session to that vault — a plain `{ vault }` there would give the user something other than what the file asked for. On a gated deployment NO verb of `confirm_workspace_binding` is available, nor is `set_secondary_vault_mode`: the workspace there is the server's own directory, shared by every caller. The tools say so; relay that rather than retrying.
- Otherwise continue.
