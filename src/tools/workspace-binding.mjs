...
export async function confirmWorkspaceBinding(registry, args = {}, seams = {}) {
  ...
  if (args.clear === true) {
    const had = readBinding(readConfig(), cwd);
    updateConfigBindings(configPath, (cfg) => withoutBinding(cfg, cwd), io);
    ...
    if (had?.locked && registry.lockedVault === had.vault) {
      // A lock the BINDING imposed goes with it. One the host imposed does
      // not — and a host lock the binding had been SHADOWING comes back,
      // whatever vault it names. The first version kept the host lock only
      // when it named the same vault as the binding, so clearing a binding
      // locked to A silently dropped a host lock on B. Codex round 2.
      releaseBindingLock(registry);
    }
    if (registry.defaultVault === had?.vault) {
    ...
    return {
      ...
      boundTo: null,
      message: had
        ? `This workspace is no longer bound to "${had.vault}". All registered vaults are available again, and the default is resolved by the usual cascade.`
        : 'This workspace had no binding; nothing changed. All registered vaults were, and remain, available.',
    };
  }
  ...
  const unknown = requested.filter((n) => typeof n !== 'string' || !known.has(n)
    // A live vault that the file no longer lists — or that only the environment
    // provides (VAULT_*), which the next session may not have either.
    || (known.get(n)?.type === 'local' && !fileNames.has(n)));
  if (unknown.length) {
    const names = unknown.map((n) => `"${safeForMessage(String(n), 60)}"`).join(', ');
    const available = [...known.keys()].join(', ') || '(none)';
    throw new Error(
  ...
  const previous = readBinding(onDisk, cwd);
  const locked = typeof args.locked === 'boolean'
    ? args.locked
    : Boolean(previous && previous.vault === primary && previous.locked);
  const next = updateConfigBindings(configPath, (cfg) => withBinding(cfg, cwd, {
    vault: primary,
    also,
    locked,
  ...
  return {
    ...
    message:
      (several
        ? `This workspace is now bound to "${primary}", with ${also.map((n) => `"${n}"`).join(', ')} also bound and addressable by name.`
        : `This workspace is now bound to "${primary}".`)
      + (opened.length
