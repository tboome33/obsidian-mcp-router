 ...
 */
export async function scanSessionFolders({ listFilesIn, vault, wikiPaths, current }) {
  ...
  try {
    ...
    if (err?.kind === 'not_found') {
      return { findings: detectSessionFolderCollision(entries).findings, skipped: null, metaDirs: [] };
    }
...
export async function refreshProjectionsForVault(vault, deps, opts = {}) {
  ...
  const sessions = sessionScan
    ? await scanSessionFolders({ listFilesIn: deps.listFilesIn, vault, wikiPaths: paths, current })
    : undefined;
