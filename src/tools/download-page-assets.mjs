 ...
 *     extracted: 24,                        // image URLs found
 *     downloaded: [{sourceUrl, savedAs, bytes}, ...],
 *     skipped:    [{sourceUrl, reason, bytes?}, ...],
...
export const TOOL_DEFINITION = {
  name: TOOL_NAME,
  description:
    'Download image assets from a web page or HTML string to a local directory, returning a manifest of saved files. Used by wiki-ingest when the user opts into `--save-assets`. SSRF-safe (pinned-IP dispatcher), size-capped per asset (10 MiB default), and filters out icons by size threshold (1 KB default). Returns a urlMap that callers use to rewrite `![](url)` references in the source markdown to local paths.',
  inputSchema: {
    type: 'object',
    properties: {
      url: {
...
export async function handleDownloadPageAssets(args = {}) {
  ...
  // Extract → cap → download.
  const allUrls = extractImageUrls(resolvedHtml, baseUrl);
  const capped = allUrls.slice(0, Math.max(0, maxAssets));
