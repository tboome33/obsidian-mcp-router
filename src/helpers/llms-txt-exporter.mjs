...
export function buildLlmsTxt({ vaultName, indexMd, pages, mode = 'index', summary }) {
  ...
  for (const page of pages) {
    ...
    // v0.59.0 — generated OKF projections are navigation, not content.
    if ((page.path === 'wiki/log.md' || /^wiki\/(?:.+\/)?index\.md$/.test(page.path))
      && String(page.content ?? '').split(/\r?\n/, 20)
        .some((l) => l.startsWith('> Generated by obsidian-mcp-router'))) continue;
