...
describe('readResource', () => {
  ...
  test('strips control characters / ANSI escapes from vault content (codex P1)', async () => {
    // Resource reads must be sanitized like get_file — control chars corrupt the
    // MCP stdio JSON stream and ANSI escapes smuggle into the agent context.
    // Escape sequences keep this test source as plain text (no raw control bytes
    ...
    // No raw control chars survive (\n \t \r would be allowed but none here).
    assert.doesNotMatch(res.contents[0].text, /[\x00-\x08\x0b\x0c\x0e-\x1f]/);
    assert.doesNotMatch(res.contents[0].text, /\x1b/); // ANSI ESC stripped
    assert.match(res.contents[0].text, /cleantext/); // BEL between was removed
  });
